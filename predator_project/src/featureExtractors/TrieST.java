...
public class TrieST<Value> {
	...
	public Value getBlackListWord(String key) {
		Node x = getBlackListWord(root, key, 0);
		if (x == null) return null;
		return (Value) x.val;
	}
	
	public Value getWordWithSpaces(String key) {
		Node x = getWordWithSpaces(root, key, 0);
		if (x == null) return null;
		return (Value) x.val;
	}
	...
	public boolean containsBlackListWord(String key) {
		return getBlackListWord(key) != null;
	}
	
	public boolean containsWordWithSpaces(String key) {
		return getWordWithSpaces(key) != null;
	}
	...
	private Node getBlackListWord(Node x, String key, int d) {
		if (x == null) return null;
	...
	private Node getWordWithSpaces(Node x, String key, int d) {
		if (x == null) return null;
		if (x.val != null) return x;
		if (d == key.length()) return x;
		char c = key.charAt(d);
		return getWordWithSpaces(x.next[c], key, d+1);
	}
